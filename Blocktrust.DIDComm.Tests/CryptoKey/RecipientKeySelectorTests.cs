...
{
    [Fact]
    public async Task Test_find_verification_key()
    {
        ...
        var key = await recipientKeySelector.FindVerificationKey(expected);
        Assert.Equal(expected, key.Id);
    }

    [Fact]
    public async Task Test_find_anon_crypto_keys()
    {
        var recipientKeySelector = new RecipientKeySelector(new DidDocResolverMock(), new BobSecretResolverMock());
        ...
        var keys = await recipientKeySelector.FindAnonCryptKeys(expected);
        Assert.Equal(expected, keys.Select(x => x.Id).ToList());
    }

    [Fact]
    public async Task Test_find_second_anon_crypto_key()
    {
        var recipientKeySelector = new RecipientKeySelector(new DidDocResolverMock(), new BobSecretResolverMock());
        ...
        var keys = await recipientKeySelector.FindAnonCryptKeys(recipient);
        Assert.Equal(expected, keys.Select(x => x.Id).ToList());
    }

    [Fact]
    public async Task Test_find_auth_crypto_keys()
    {
        var recipientKeySelector = new RecipientKeySelector(new DidDocResolverMock(), new BobSecretResolverMock());
        ...
        var (from, to) = await recipientKeySelector.FindAuthCryptKeys(sender, recipient);
    ...
    [Fact]
    public async Task Test_DID_is_passed_to_methods()
    {
        var recipientKeySelector = new RecipientKeySelector(new DidDocResolverMock(), new BobSecretResolverMock());

        var exception1 = await Assert.ThrowsAsync<Exception>(async () => await recipientKeySelector.FindVerificationKey(JWMFixture.ALICE_DID));
        Assert.Equal("'DID URL' is expected as a signature verification key. Got: did:example:alice", exception1.Message);

        var exception2 = await Assert.ThrowsAsync<Exception>(async () => await recipientKeySelector.FindAuthCryptKeys(JWMFixture.ALICE_DID, new List<string> { JWMFixture.BOB_DID }));
        Assert.Equal("'DID URL' is expected as a sender key. Got: did:example:alice", exception2.Message);

        var exception3 = await Assert.ThrowsAsync<Exception>(async () => await recipientKeySelector.FindAnonCryptKeys(new List<string> { JWMFixture.BOB_DID }));
        Assert.Equal("'DID URL' is expected as a recipient key. Got: did:example:bob", exception3.Message);
    }

    [Fact]
    public async Task Test_key_not_found()
    {
        var recipientKeySelector = new RecipientKeySelector(new DidDocResolverMock(), new BobSecretResolverMock());
        var didUrl = "did:example:bob#key-x25519-4";
        var expected = $"The Secret '{didUrl}' not found";

        var exception1 = await Assert.ThrowsAsync<SecretNotFoundException>(async () => (await recipientKeySelector.FindAnonCryptKeys(new List<string> { didUrl })).ToList());
        Assert.Equal(expected, exception1.Message);

        var exception2 = await Assert.ThrowsAsync<SecretNotFoundException>(async () => (await recipientKeySelector.FindAuthCryptKeys("did:example:alice#key-x25519-1", new List<string> { didUrl })).Item2.ToList());
        Assert.Equal(expected, exception2.Message);
    }

    [Fact]
    public async Task Test_verification_method_not_found()
    {
        var recipientKeySelector = new RecipientKeySelector(new DidDocResolverMock(), new BobSecretResolverMock());
        ...
        var exception1 = await Assert.ThrowsAsync<DidUrlNotFoundException>(async () => await recipientKeySelector.FindAuthCryptKeys(didUrl, new List<string> { didUrl }));
        Assert.Equal(expected, exception1.Message);

        var exception2 = await Assert.ThrowsAsync<DidUrlNotFoundException>(async () => await recipientKeySelector.FindVerificationKey(didUrl));
        Assert.Equal(expected, exception2.Message);
    }
    ...
    public async Task Test_DID_Doc_not_resolved()
    {
        ...
        var exception1 = await Assert.ThrowsAsync<DidUrlNotFoundException>(async () => await recipientKeySelector.FindVerificationKey(didUrl));
        Assert.Equal(expected, exception1.Message);

        var exception2 = await Assert.ThrowsAsync<DidUrlNotFoundException>(async () => await recipientKeySelector.FindAuthCryptKeys(didUrl, new List<string>()));
        Assert.Equal(expected, exception2.Message);
    }
    ...
    public async Task Test_incompatible_Crypto()
    {
        ...
        var exception1 = await Assert.ThrowsAsync<IncompatibleCryptoException>(async () => (await recipientKeySelector.FindAuthCryptKeys(charlieDIDUrl, new List<string> { bobDIDUrl })).Item2.ToList());
        Assert.Equal("The recipient '" + bobDIDUrl + "' curve is not compatible to 'X25519'", exception1.Message);
    }

    [Fact]
    public async Task Test_find_anoncrypt_unpack_recipient_private_keys_positive_single_key()
    {
        ...
        {
            var res = (await keySelector.FindAnonCryptKeys(new List<string> { vm.Kid })).Select(it => it.Jwk).ToList();
    ...
    [Fact]
    public async Task Test_find_anoncrypt_unpack_recipient_private_keys_different_curves()
    {
        var keySelector = new RecipientKeySelector(new DidDocResolverMock(), TestUtils.GetSecretsResolver(TestUtils.Person.BOB));
