...
public class EdDsaParameters
{
    ...
    {
        if ((D == null || D.Length == 0) && (X == null || X.Length == 0)) throw new Exception("Invalid EdDSA parameters - missing keys");
        
        if (D != null)
        {
            if (Curve == ExtendedSecurityAlgorithms.Curves.Ed25519 && (D.Length != 32 && D.Length != 32*2)) throw new Exception("Invalid key length. Must be 32 bytes.");
            if (Curve == ExtendedSecurityAlgorithms.Curves.Ed448 && (D.Length != 57 && D.Length != 57*2)) throw new Exception("Invalid key length. Must be 57 bytes.");
        }

        if (X != null)
        {
            if (Curve == ExtendedSecurityAlgorithms.Curves.Ed25519 && X.Length != 32) throw new Exception("Invalid key length. Must be 32 bytes.");
            if (Curve == ExtendedSecurityAlgorithms.Curves.Ed448 && X.Length != 57) throw new Exception("Invalid key length. Must be 57 bytes.");
        }
    }
}
